...
while [ -z "$bd" ]
do
	bd=$(whiptail --title "Connexion à la base de données" --inputbox "Veuillez saisir le nom de la base de données" 10 50 3>&1 1>&2 2>&3)
	if [ $? -ne 0 ]
	then
		echo "Vous avez quittez l'installation de ASTRE"
		exit -1
...
echo "Icon=$install/ASTRE.png" >> $HOME/Bureau/ASTRE.desktop
